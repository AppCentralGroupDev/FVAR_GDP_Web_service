...
@app.post("/iterate/{dataset}")
async def iterate(dataset: str, request: Request, file: bytes = File(...)):
    print('[+] Initiate Monitoring')
    item = io.BytesIO(file)
    message = "Success"
    print(item)
    ...
    SERVICE.iterate(dataset_name=dataset, new_rows=pd.read_csv(item))
    return message
